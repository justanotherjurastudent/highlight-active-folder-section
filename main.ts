	...
	updateStyles() {
		...
		const styleContent = `
            ...
            .highlighted-folder > .nav-folder-title {
                ${
					this.settings.highlightFolderTitleColor
                ...
                font-weight: ${
					this.settings.highlightedFolderFontWeight
				}${important};
            }
            .highlighted-parent-folder {
                background-color: ${
					this.settings.highlightedParentFolderColor
				}${important};
                border-radius: ${
            ...
            .highlighted-parent-folder > .nav-folder-title {
                color: ${
					this.settings.highlightedParentFolderTextColor
				}${important};
                font-weight: ${
					this.settings.highlightedParentFolderFontWeight
				}${important};
                border-radius: ${
					this.settings.highlightedParentFolderBorderRadius
				}${important};
...
class FolderHighlighterSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const createColorSetting = (
			name: string,
			desc: string,
			key: keyof FolderHighlighterSettings
		) => {
			...
			const match = (savedColor as string).match(
				/rgba\((\d+), (\d+), (\d+), ([\d.]+)\)/
			);
			if (match) {
				alphaValue = parseFloat(match[4]);
			...
			new Setting(containerEl)
				...
				.addText((text) => {
					...
					colorPicker.addEventListener(
						...
						async (event: any) => {
							...
							(this.plugin.settings as any)[key] =
								`rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${alphaValue})` as string;
							await this.plugin.saveSettings();
					...
					transparencySlider.title =
						"Move the slider to adjust transparency";
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						...
						if (!value) {
							this.plugin.settings.highlightedFolderTitleColor =
								"#ffffff00"; // Reset background color
						}
						await this.plugin.saveSettings();
						this.display();
			...
			new Setting(containerEl)
				...
				.addDropdown((dropdown) =>
					dropdown
						...
						.setValue(
							this.plugin.settings
								.highlightedParentFolderFontWeight || "700"
						)
						.onChange(async (value) => {
