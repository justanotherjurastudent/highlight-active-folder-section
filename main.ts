...
export default class FolderHighlighter extends Plugin {
	...
	async onload() {
		...
		this.app.workspace.onLayoutReady(() => {
			// Längere Wartezeit, um sicherzustellen, dass Obsidian vollständig geladen ist
			setTimeout(() => {
				...
				// Ereignisse manuell auslösen - dies ist der Teil, der funktioniert
				...
				// Nochmals die Hervorhebungsfunktion aufrufen
				setTimeout(() => {
					this.highlightFolders();
					console.log(
					...
					// Fokus zurück zum Editor setzen
					const activeView =
			...
			}, 1500); // Längere Wartezeit zum vollständigen Laden
		});
	}
		...
		if (currentFolder) {
			...
			for (const intermediateFolder of intermediateFolders) {
				if (
					...
					(!this.settings.highlightParentFolder ||
						intermediateFolder !==
							this.getRootFolderInPath(activeFile.path))
				) {
	...
	updateStyles() {
		...
		if (this.settings.useImportantTags) {
			const addImportant = (variable: string) => {
				const value =
					getComputedStyle(rootEl).getPropertyValue(variable);
...
class FolderHighlighterSettingTab extends PluginSettingTab {
		...
		new Setting(settingContainer)
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value) => {
						this.plugin.settings.highlightedFolderFontWeight =
							value;
						await this.plugin.saveSettings();
						this.plugin.updateStyles();
					})
			...
			new Setting(settingContainer)
				...
				.addDropdown((dropdown) =>
					dropdown
						...
						.setValue(
							this.plugin.settings
								.highlightedParentFolderFontWeight
						)
						.onChange(async (value) => {
				...
				.addSlider((slider) =>
					slider
						...
						.setValue(
							parseInt(
								this.plugin.settings
									.highlightedParentFolderBorderRadius
							)
						)
						.onChange(async (value) => {
	...
	private createColorSetting(
		...
		const defaultKey =
			`${prefix}${baseKey}` as keyof typeof DEFAULT_SETTINGS;
	...
	private getCurrentThemeSetting(key: string): boolean {
		...
		return this.plugin.settings[
			themeKey as keyof FolderHighlighterSettings
		] as boolean;
	}

	private setThemeSetting(key: string, value: boolean) {
		(this.plugin.settings[
			`dark${key}` as keyof FolderHighlighterSettings
		] as boolean) = value;
		(this.plugin.settings[
	...
	addColorComparison(
		...
		comparisonEl.createSpan({
			text: `Im ${
				this.plugin.settings.editingDarkTheme ? "hellen" : "dunklen"
			} Theme: `,
		});
	...
	updateColorComparison(
		...
		const existingComparison =
			containerEl.querySelector(".color-comparison");
